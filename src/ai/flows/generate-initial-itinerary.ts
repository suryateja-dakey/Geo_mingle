...
import { getPlacePhotoUrlTool } from '../tools/get-place-photo';
...
const ActivitySchema = z.object({
  ...
  imageUrl: z.string().url().optional().describe('The URL of an image for this activity. This should be generated by calling the getPlacePhotoUrlTool.'),
});
...
4. 'imageUrl': You MUST call the 'getPlacePhotoUrl' tool with a query combining the 'location' and 'city' to get a photo URL for the location.

If the activity is a meal like breakfast, lunch or dinner, you MUST suggest a specific, well-known restaurant in that city and use its name for the 'location'. For example, if suggesting biryani in Hyderabad, you might set the location to "Paradise Biryani". You MUST still call the 'getPlacePhotoUrl' for the restaurant.

If an activity does not have a specific location (e.g., "take a walk"), you MUST OMIT the 'location' and 'imageUrl' fields. Do NOT invent locations.`,
});
